...
import { getProducts, getAllCategories, getProductsByCategory } from "@/actions/catalogActions";
...
export default async function ProductsPage({ searchParams }: Props) {
  ...
  const [allCategories, initialProducts] = await Promise.all([
    getAllCategories(),
    selectedCategory ? getProductsByCategory(selectedCategory) : getProducts(1, 50),
  ]);
  ...
  return (
    <div className="container py-8 space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <Link href="/products" className={!selectedCategory ? "text-foreground font-medium" : "hover:text-foreground"}>
          Products
        </Link>
        {selectedCategory && (
          <>
          ...
          {lastProduct && (
            <Card className="border shadow-sm overflow-hidden hidden sm:block">
              <CardHeader className="p-3 bg-emerald-600 text-white">
                <span className="text-xs font-bold uppercase">Featured Item</span>
              </CardHeader>
              <CardContent className="p-4 space-y-3">
                <div className="h-32 bg-slate-50 flex items-center justify-center p-2 rounded">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={lastProduct.imageFile ? `/images/product/${lastProduct.imageFile}` : "/images/placeholder.png"}
                    alt={lastProduct.name}
                    className="max-h-full max-w-full object-contain"
                  />
                </div>
                <h4 className="font-semibold text-sm line-clamp-1">{lastProduct.name}</h4>
                <p className="text-xs text-muted-foreground line-clamp-2">{lastProduct.description}</p>
                <div className="font-bold text-emerald-600 text-base">
                  ${Number(lastProduct.price).toFixed(2)}
                </div>
                <Link href={`/products/${lastProduct.id}`} className="block">
