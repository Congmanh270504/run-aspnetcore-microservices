...
export default async function AdminDashboardPage() {
  ...
  const totalRevenue = orders.reduce((sum, o) => {
    const orderTotal = o.orderItems?.reduce((sub, i) => sub + i.price * i.quantity, 0) || 0;
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Real-time overview of products, categories, and orders across all microservices.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-sm">
          <CardHeader className="p-5 border-b flex flex-row items-center justify-between">
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Package className="h-4 w-4 text-primary" /> Recent Products
            </CardTitle>
              ...
              <div key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded bg-slate-100 p-1 flex items-center justify-center border shrink-0">
                    ...
                    <img
                      src={p.imageFile ? `/images/product/${p.imageFile}` : "/images/placeholder.png"}
                      alt={p.name}
                  ...
                  <div className="truncate">
                    ...
                    <p className="text-xs text-muted-foreground">{p.category?.[0] || "General"}</p>
                  </div>
                </div>
        ...
        <Card className="shadow-sm">
          <CardHeader className="p-5 border-b flex flex-row items-center justify-between">
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <ShoppingCart className="h-4 w-4 text-amber-500" /> Recent Orders
            </CardTitle>
          ...
          <CardContent className="p-0 divide-y">
            {orders.length > 0 ? (
              orders.slice(0, 5).map((o) => {
                const total = o.orderItems?.reduce((sum, i) => sum + i.price * i.quantity, 0) || 0;
                return (
                  <div key={o.id || o.orderName} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    <div>
                      <p className="font-semibold text-sm text-slate-900">
                        {o.shippingAddress?.firstName} {o.shippingAddress?.lastName}
                      </p>
                      <p className="text-xs text-muted-foreground">{o.orderName || "Online Order"}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-sm text-emerald-600">${total.toFixed(2)}</p>
