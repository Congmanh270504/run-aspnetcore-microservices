 ...
 * 2. Cookie duyphat-rows-per-page (từ ThemeProvider) - fallback
 * 3. Default value
 */
export async function getRowsPerPage(
    ...
    if (paramPageSize) {
        ...
        if ([20, 30, 40, 50, 100].includes(val)) return val;
    }

    // Đọc từ cookie (set bởi ThemeProvider)
    try {
        const cookieStore = await cookies();
        const cookieVal = cookieStore.get("duyphat-rows-per-page")?.value;
        if (cookieVal) {
